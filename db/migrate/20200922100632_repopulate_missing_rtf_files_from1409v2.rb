class RepopulateMissingRtfFilesFrom1409v2 < ActiveRecord::Migration[6.0]
  ...
  def up
    Command.where('request_body LIKE ? AND created_at > ? AND created_at < ?', '%BuildClaimDetailsFile%', DateTime.parse('14/9/2020 00:00:00'), DateTime.parse('14/9/2020 23:59:59')).all.each do |command|
      ...
      announce "Adding new RTF file for claim id #{claim.id} reference #{claim.reference}"
      input_data = file_command['data']
      url = URI.parse(input_data.delete('data_url')).tap {|u| u.query = nil }.to_s
      uploaded_file = claim.uploaded_files.create(input_data.merge(import_file_url: url, import_from_key: input_data.delete('data_from_key')))
