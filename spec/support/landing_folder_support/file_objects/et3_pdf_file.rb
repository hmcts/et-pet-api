      ...
      class Et3PdfFile < Base # rubocop:disable Metrics/ClassLength
        ...
        def has_representative_for?(representative, errors: [], indent: 1)
          ...
          validate_fields section: :representative, errors: errors, indent: indent do
            ...
            expect(field_values).to include '7.3 number or name' => address.try(:[], :building) || ''
            expect(field_values).to include '7.3 street' => address.try(:[], :street) || ''
            expect(field_values).to include '7.3 town city' => address.try(:[], :locality) || ''
            expect(field_values).to include '7.3 county' => address.try(:[], :county) || ''
            expect(field_values).to include '7.3 postcode' => address.try(:[], :post_code).try(:tr, ' ', '') || ''
