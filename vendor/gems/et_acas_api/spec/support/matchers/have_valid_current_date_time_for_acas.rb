...
RSpec::Matchers.define :have_valid_current_date_time_for_acas do |expected|
  time_zone = Rails.configuration.et_acas_api.server_time_zone
  sub_matcher = ::RSpec::Matchers::BuiltIn::BeWithin.new(10.seconds).of(Time.find_zone!(time_zone).now)
  match do |actual|
    ...
    actual_date = Time.find_zone!(time_zone).parse(decrypted)
