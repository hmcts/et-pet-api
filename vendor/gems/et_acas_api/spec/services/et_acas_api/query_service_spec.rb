...
RSpec.describe EtAcasApi::QueryService do
  ...
  describe '.dispatch' do
    context 'with a valid query' do
      ...
      it 'creates a new instance of the query' do
        ...
        expect(query_class).to have_received(:new).with(my_param1: 'test', 'my_param2': 'test2', acas_api_service: instance_of(EtAcasApi::V1::AcasApiService))
      end
      ...
      it 'uses v1 of the api when configured' do
        # Arrange - setup the fake service class
        fake_service_class = class_spy('EtAcasApi::V1::AcasApiService').as_stubbed_const
        # Act - call dispatch
        result = service.dispatch query: 'Dummy',
                                  root_object: root_object,
                                  my_param1: 'test',
  ...
  describe '.supports_multi?' do
    it 'is delegates to the service class and returns a negative value' do
      # Arrange - setup the fake service class
      fake_service_class = class_spy('EtAcasApi::V1::AcasApiService', supports_multi?: false).as_stubbed_const
      # Act - call dispatch
      result = service.supports_multi? api_version: 1
